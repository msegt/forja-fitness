...
// Kept intentionally short — every extra token costs quota
...
// Only send what Gemini actually needs — drop anything that bloats token count
function slimProfile(profileData: unknown, daysPerWeek: number) {
  ...
  return {
    ...
    // age derived from DOB if present, avoids sending raw DOB string
    age: p.dateOfBirth
...
export async function generateWorkoutPlan(profileData: unknown, daysPerWeek: number): Promise<WorkoutPlan> {
  ...
  const response = await model.generateContent(`${PLAN_PROMPT}\n\nUser: ${JSON.stringify(slim)}`);
...
export async function chatWithForja(
  ...
  if (!client) {
    return "I\u2019m here for you. Stay consistent and focus on quality movement today.";
  }

  // Slim system prompt — no plan JSON, just enough to personalise the reply
